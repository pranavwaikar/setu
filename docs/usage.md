   ...
   * **If using raw Docker Compose**: Run the following command:
    ...
    * **If using Coolify**: Point Coolify to this repository, select `docker-compose.yml`, and configure the service domains:
      ...
      * **`api`**: **Keep the domain field empty**.
      ...
      > Because Coolify's compose parser does not interpolate variables in `labels`, you must configure wildcard subdomain routing directly in Coolify's Traefik Proxy configuration editor:
      > 1. Go to **Servers** -> **[Your Server]** -> **Proxy** tab.
      > 2. Navigate to **Dynamic Configurations** (or edit the Proxy configuration file) and add/edit a file (e.g., `setu.yaml`).
      > 3. Add the following YAML configuration (replace `setu.yourdomain.com` with your actual domain):
      >    ```yaml
      >    http:
      >      routers:
      >        setu-tunnel-https:
