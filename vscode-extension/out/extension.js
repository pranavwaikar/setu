...
function activate(context) {
    ...
    function startTunnelProcess(subdomain, port) {
        ...
        const child = (0, child_process_1.spawn)('setu', ['expose', port, '--subdomain', subdomain], {
            shell: true,
            env: { ...process.env }
        });
    ...
    context.subscriptions.push(vscode.commands.registerCommand('setu.startTunnel', (item) => {
    ...
    // Command: Stop Tunnel
    context.subscriptions.push(vscode.commands.registerCommand('setu.stopTunnel', (item) => {
        if (!item || item.isPlaceholder) {
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('setu.refreshTunnels', () => {
        treeDataProvider.refresh();
        updateStatusBar(statusBarItem, activeTunnels);
        vscode.window.showInformationMessage('Setu tunnel list refreshed.');
    }));
    // Command: Expose active workspace port
    context.subscriptions.push(vscode.commands.registerCommand('setu.exposeWorkspace', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('setu.openInspector', () => {
        if (inspectorPanel) {
            inspectorPanel.reveal(vscode.ViewColumn.Active);
            return;
        }
        inspectorPanel = vscode.window.createWebviewPanel('setuInspector', 'Setu Traffic Inspector', vscode.ViewColumn.Active, {
            enableScripts: true,
    ...
    context.subscriptions.push(vscode.commands.registerCommand('setu.login', async () => {
        ...
        await vscode.window.withProgress({
            ...
            if (result.valid) {
                ...
                try {
                    ...
                    treeDataProvider.refresh();
                    updateLoginContext();
                    vscode.window.showInformationMessage(`Setu: Logged in successfully as ${result.firstName || ''} ${result.lastName || ''} (${result.email || ''})!`);
                }
                catch (e) {
    ...
    // Command: Logout
    context.subscriptions.push(vscode.commands.registerCommand('setu.logout', async () => {
        const confirm = await vscode.window.showWarningMessage('Are you sure you want to log out of Setu?', 'Yes', 'No');
        if (confirm !== 'Yes')
            return;
        const homedir = os.homedir();
        const configPath = path.join(homedir, '.setu', 'config.json');
    ...
    context.subscriptions.push(vscode.commands.registerCommand('setu.addMapping', async () => {
        const subdomain = await vscode.window.showInputBox({
    ...
    context.subscriptions.push(vscode.commands.registerCommand('setu.editMapping', async (item) => {
        if (!item || item.isPlaceholder)
            return;
        const port = await vscode.window.showInputBox({
    ...
    context.subscriptions.push(vscode.commands.registerCommand('setu.deleteMapping', async (item) => {
        if (!item || item.isPlaceholder)
            return;
        const confirm = await vscode.window.showWarningMessage(`Are you sure you want to delete the mapping for ${item.mapping.subdomain}?`, 'Yes', 'No');
        if (confirm !== 'Yes')
            return;
        const homedir = os.homedir();
        const configPath = path.join(homedir, '.setu', 'config.json');
...
function deactivate() { }
function updateStatusBar(statusBarItem, activeTunnels) {
...
function saveMappingToConfig(subdomain, port) {
    const homedir = os.homedir();
    const configDir = path.join(homedir, '.setu');
    const configPath = path.join(configDir, 'config.json');
    try {
...
class TunnelTreeProvider {
    ...
    getChildren(element) {
        ...
        if (!config || !config.api_key) {
            return Promise.resolve([
                new TunnelItem("Not logged in", "Click the Key icon to login", vscode.TreeItemCollapsibleState.None, { subdomain: '', port: '' }, false, true)
            ]);
        }
        if (!config.port_mappings || config.port_mappings.length === 0) {
            return Promise.resolve([
                new TunnelItem("No active mappings", "Click the + icon to add a mapping", vscode.TreeItemCollapsibleState.None, { subdomain: '', port: '' }, false, true)
            ]);
        }
        const items = config.port_mappings.map(mapping => {
            const isConnected = this.activeTunnels.has(mapping.subdomain);
            return new TunnelItem(`${mapping.subdomain}.${config.gateway || 'setu.helios-logic.com'}`, `→ localhost:${mapping.port}`, vscode.TreeItemCollapsibleState.None, mapping, isConnected);
        });
...
class TunnelItem extends vscode.TreeItem {
    constructor(label, description, collapsibleState, mapping, isConnected, isPlaceholder = false) {
...
function validateApiKeyWithGateway(gateway, apiKey) {
